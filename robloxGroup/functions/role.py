...
async def change(group: Type[Bot], user_id: Union[int, str], role_id: Union[int, str]) -> Type[DotDict]:
...
async def guilds_user(group: Type[Bot], user_id: Union[int, str]) -> Type[DotDict]:
    async with aiohttp.ClientSession() as session:
